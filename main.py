...
for post in sorted_posts:
    with open('db.csv', 'a') as f:
        f.write(str(post['source'])+','+str(post['date'])+','+str(post['caption']).replace('\n', ' ')+','+str(post['media'])+','+str(post['link'])+'\n')
    print(post['source'],post['date'],post['caption'].replace('\n', ' '),post['media'],post['link'])